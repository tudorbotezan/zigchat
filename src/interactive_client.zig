...
pub const InteractiveClient = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, channel: []const u8, relay_url: []const u8) Self {
        ...
        const keypair = nostr_crypto.KeyPair.generate();
    ...
    fn sendMessage(self: *Self, content: []const u8) !void {
        ...
        var formatted_content_buf: [2048]u8 = undefined;
        const formatted_content = try std.fmt.bufPrint(&formatted_content_buf, "[{s}]: {s}", .{self.username, content});
        ...
        var event = try nostr_crypto.NostrEvent.create(
            self.keypair,
